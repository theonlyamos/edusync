    ...
    parameters: {
      ...
      properties: {
        explanation: {
          type: 'string',
          description:
            "The complete text explanation that will accompany the visual. This explanation must be easy to understand and contain NO technical jargon (e.g., 'React', 'useState', 'JavaScript').",
        },
        code: {
          type: 'string',
          description:
            'The complete, runnable, and self-contained code snippet. It must not contain any comments. For React, the code MUST use `React.createElement()` syntax and never JSX.',
        },
        library: {
          type: 'string',
...
export type VisualizeGenerationInput = {
...
export type VisualizeGenerationResult = {
  explanation: string
  code: string
  library: 'p5' | 'three' | 'react'
}

function normalizeVizResult(explanation: unknown, code: unknown, library: unknown): VisualizeGenerationResult {
  const lib =
    library === 'p5' || library === 'three' || library === 'react' ? library : ('react' as const)
  const exp = typeof explanation === 'string' ? explanation : ''
  const c = typeof code === 'string' ? code : ''
  if (!c.trim()) {
    throw new Error('Model did not return visualization code')
  }
