...
import { useEffect, useRef, useState, Suspense, useCallback, useMemo, useContext, useReducer } from 'react';
...
export const InteractiveAITutorComponent = ({ onSessionStarted, onSessionEnded }: InteractiveAITutorProps) => {
  ...
  const setVisualizations = useCallback((vizs: Visualization[] | ((prev: Visualization[]) => Visualization[])) => {
    // For backward compatibility with functional updates
    if (typeof vizs === 'function') {
      // This case requires reading current state, so we handle it specially
      console.warn('Functional setVisualizations updates should be migrated to dispatch actions');
    }
  }, []);
